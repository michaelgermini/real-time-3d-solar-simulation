...
// Solar Panel class with enhanced features
class SolarPanel {
public:
    ...
    SolarPanel(PanelType type, const glm::vec3& pos, const glm::vec2& sz)
        : type(type), position(pos), size(sz), efficiency(0.22f), powerOutput(400.0f),
          ...
          health(1.0f), maintenanceLevel(1.0f), isSelected(false) {
        GenerateGeometry();
        InitializeRandom();
    }
    ...
    void Update(float deltaTime, float timeOfDay, float solarIntensity, float ambientTemp, float windSpeed) {
        ...
        float basePower = powerOutput * (1.0f - temperature * 0.004f); // Temperature effect
        float dirtEffect = 1.0f - dirtLevel * 0.3f;
        float healthEffect = health;
        float maintenanceEffect = maintenanceLevel;
        
        currentPower = basePower * angleEfficiency * dirtEffect * healthEffect * maintenanceEffect * solarIntensity;
        
        // Energy accumulation
        dailyEnergyOutput += currentPower * deltaTime / 3600.0f; // Convert to kWh
        
        // Age and degradation
        age += deltaTime / 86400.0f; // Age in days
        health = std::max(0.5f, 1.0f - age * 0.001f); // 0.1% degradation per day
        
        // Maintenance simulation
        if (dirtLevel > 0.8f && maintenanceLevel < 0.9f) {
            maintenanceLevel = std::min(1.0f, maintenanceLevel + deltaTime * 0.01f);
        }
    }
    ...
    bool IntersectsRay(const glm::vec3& rayOrigin, const glm::vec3& rayDirection) const {
        ...
        // Panel bounds in local space (centered at origin, size from -0.5 to 0.5)
...
private:
    ...
    float dailyEnergyOutput, monthlyEnergyOutput, yearlyEnergyOutput;
    float age, health, maintenanceLevel;
    bool isSelected;
...
// Camera class with enhanced controls
class Camera {
public:
...
int main() {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        float windSpeed = 5.0f + sin(timeOfDay * 4.0f * 3.14159f) * 3.0f;
